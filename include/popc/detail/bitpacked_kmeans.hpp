...
public:
    ...
    explicit bitpacked_dataset(popc::dataset const& ds)
        : num_instances_{ds.num_instances()},
          num_attributes_{ds.num_attributes()},
        ...
        for (std::size_t i = 0; i < num_instances_; ++i) {
            ...
            for (auto it = ds.cbegin(i); it != ds.cend(i); ++it) {
                if (*it) {
                    data_[base + bit / bits_per_word] |=
                        (word_type{1} << (bit % bits_per_word));
                }
                ++bit;
            }
        }
    ...
    [[nodiscard]] std::size_t words_per_instance() const noexcept {
        return words_per_instance_;
    }
...
[[nodiscard]] inline std::size_t hamming_distance(
    std::span<bitpacked_dataset::word_type const> a,
...
// drain.
[[nodiscard]] inline std::vector<std::size_t> bitpacked_kmodes_seed(
    bitpacked_dataset const& data, std::size_t k, std::size_t max_iterations,
    std::uint64_t seed) {
    ...
    for (std::size_t iter = 0; iter < max_iterations; ++iter) {
        ...
        for (std::size_t c = 0; c < k; ++c) {
            ...
            std::fill_n(centroids.begin() + static_cast<std::ptrdiff_t>(cent_off), wpi,
                        word_type{0});
            if (size == 0) {
                continue;
            }
...
[[nodiscard]] inline std::vector<std::size_t> bitpacked_kmodes_seed(
    popc::dataset const& ds, std::size_t k, std::size_t max_iterations = 32,
